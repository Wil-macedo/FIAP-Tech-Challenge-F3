...
file_path = st.sidebar.text_input(
    "Insira o caminho do arquivo CSV:",
    os.path.join(os.path.abspath(os.path.curdir), "DATASET", "base.csv"),
)
...
# t-SNE scatter plot
axes[0].scatter(
    X_reduced_tsne[:, 0], X_reduced_tsne[:, 1], c=y, cmap="coolwarm", linewidths=2
)
...
# PCA scatter plot
axes[1].scatter(
    X_reduced_pca[:, 0], X_reduced_pca[:, 1], c=y, cmap="coolwarm", linewidths=2
)
...
axes[2].scatter(
    X_reduced_svd[:, 0], X_reduced_svd[:, 1], c=y, cmap="coolwarm", linewidths=2
)
...
for train_index, test_index in sss.split(undersample_X, undersample_y):
    undersample_Xtrain, undersample_Xtest = (
        undersample_X.iloc[train_index],
        undersample_X.iloc[test_index],
    )
    undersample_ytrain, undersample_ytest = (
        undersample_y.iloc[train_index],
        undersample_y.iloc[test_index],
    )
...
log_reg = (
    LogisticRegression()
)  # Example classifier (can replace with other classifiers)
for train, test in sss.split(undersample_Xtrain, undersample_ytrain):
    # Create pipeline using NearMiss and Logistic Regression
    undersample_pipeline = make_pipeline(
...
def plot_learning_curve_streamlit(
    estimator1,
    estimator2,
    estimator3,
    estimator4,
    ...
    ax1.plot(
        train_sizes, train_scores_mean, "o-", color="#ff9124", label="Training score"
    )
    ax1.plot(
        train_sizes,
        test_scores_mean,
...
fig = plot_learning_curve_streamlit(
    log_reg,
    knears_neighbors,
    svc,
    tree_clf,
...
def graph_roc_curve_multiple(
    log_fpr, log_tpr, knear_fpr, knear_tpr, svc_fpr, svc_tpr, tree_fpr, tree_tpr
):
